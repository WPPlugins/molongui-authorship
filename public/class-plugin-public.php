...
class Plugin_Public
{
	...
	private function load_premium_dependencies()
	{
		 ...
		 */
		if ( $settings['enable_guest_archive'] ) require_once( MOLONGUI_AUTHORSHIP_DIR . '/premium/includes/virtualpages.php' );
	}
	...
	public function enqueue_styles()
	{
		...
		if( !is_premium() )
		{
			$fpath = '/public/css/molongui-authorship.3aa9.min.css';
		...
		{
			$fpath = '/premium/public/css/molongui-authorship-premium.882b.min.css';
			if ( file_exists( MOLONGUI_AUTHORSHIP_DIR . $fpath ) )
				wp_enqueue_style( $this->plugin_name, MOLONGUI_AUTHORSHIP_URL . $fpath, array(), $this->version, 'all' );
		}
	}
	...
	public function enqueue_scripts()
	{
        ...
        wp_enqueue_script( 'molongui-resizesensor', MOLONGUI_AUTHORSHIP_URL . '/public/js/vendor/element-queries/ResizeSensor.js', array( 'jquery' ), $this->version, false );
        wp_enqueue_script( 'molongui-elementqueries', MOLONGUI_AUTHORSHIP_URL . '/public/js/vendor/element-queries/ElementQueries.js', array( 'jquery' ), $this->version, false );
	...
	public function filter_author_link( $link )
	{
         ...
         */
        $db = debug_backtrace();
        if ( $db[7]['function'] == 'molongui_shortcode_display_author_list'
             and
             $db[6]['function'] == 'get_authors' and $db[6]['class'] == 'Molongui\Authorship\Includes\Guest_Author'
             and
             $db[5]['function'] == 'get_author_data' and $db[5]['class'] == 'Molongui\Authorship\Includes\Guest_Author'
             and
             $db[4]['function'] == 'get_author_posts_url'
        ) return $link;
		...
		if ( $guest_id = get_post_meta( $post->ID, '_molongui_guest_author_id', true ) )
		{
			...
			if ( is_premium() and $settings['enable_guest_archive'] )
			{
				// Get guest author.
	...
	public function add_author_meta()
	{
		...
		if ( $author_id = get_post_meta( $post->ID, '_molongui_guest_author_id', true ) )
		{
			...
			$author_gp   = get_post_meta( $author_id, '_molongui_guest_author_googleplus', true );
		}
		else
		{
			// Registered author.
			...
			$author_gp   = get_the_author_meta( 'molongui_author_googleplus' );
		}
