...
class Plugin_Admin
{
	...
	public function enqueue_styles()
	{
		...
		if( !is_premium() )
		{
			$fpath = '/admin/css/molongui-authorship-admin.c722.min.css';
		...
		{
			$fpath = '/premium/admin/css/molongui-authorship-premium-admin.aebb.min.css';
			if ( file_exists( MOLONGUI_AUTHORSHIP_DIR . $fpath ) )
				wp_enqueue_style( $this->plugin_name, MOLONGUI_AUTHORSHIP_URL . $fpath, array(), $this->version, 'all' );
		}
	}
	...
	{
        // Enqueue 'Element Queries' only on ?page=molongui_authorship (for box previewing on 'box' tab)
        if( $hook == 'settings_page_molongui_authorship' )
        {
		...
		if( !is_premium() )
		{
			$fpath = '/admin/js/molongui-authorship-admin.437e.min.js';
			if ( file_exists( MOLONGUI_AUTHORSHIP_DIR . $fpath ) )
				wp_enqueue_script( $this->plugin_name, MOLONGUI_AUTHORSHIP_URL . $fpath, array( 'jquery', 'wp-color-picker' ), $this->version , true );
		}
		else
		{
			$fpath = '/premium/admin/js/molongui-authorship-premium-admin.437e.min.js';
			if ( file_exists( MOLONGUI_AUTHORSHIP_DIR . $fpath ) )
				wp_enqueue_script( $this->plugin_name, MOLONGUI_AUTHORSHIP_URL . $fpath, array( 'jquery', 'wp-color-picker' ), $this->version, true );
		}
